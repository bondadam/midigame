window.midiGame = new function() {
    ...
    this.renderGame = function() {
        ...
        var cycleHue = (phase) => {
            ...
            var center = 0.5;
            var width = 0.5;
            red = Math.sin(this.currentTime * 0.0001 + 2 + phase) * width + center;
            green = Math.sin(this.currentTime * 0.0001 + 0 + phase) * width + center;
            blue = Math.sin(this.currentTime * 0.0001 + 4 + phase) * width + center;
            return new BABYLON.Color3(red, green, blue);
        }

        for (var i = 0; i < extrudedTracks.length; i++) {
            var cycledColor = cycleHue(i);
            extrudedTracks[i].material.diffuseColor = cycledColor;
            extrudedTracks[i].material.specularColor = cycledColor;
            extrudedTracks[i].material.emissiveColor = cycledColor;
        }
        var newDirection = Math.random();

        if (newDirection > 0.999) {
            this.direction = -this.direction;
        } else if (newDirection < 0.001) {
            this.directionX = -this.directionX;
        }
        /*
        if (this.skyBox.position.y > 250) {
            direction = -0.05;
        } else if (this.skyBox.position.y < -250) {
            direction = 0.05;
        }

        if (this.skyBox.position.x > 250) {
            directionX = -0.05;
        } else if (this.skyBox.position.x < -250) {
            directionX = 0.05;
        }*/

        this.skyBox.rotation.x += this.directionX;
        this.skyBox.rotation.y += this.direction;
    };
    ...
    this.createMenuScene = function() {
        ...
        */
        ...
        exampleBtn.onPointerClickObservable.add(async () => {
            await checkLoadingSuccess("/media/rhcp.mid");
        });
        ...
        await MidiConvert.load(url, (midi) => {
            ...
            for (var i = 0; i < midi.tracks.length; i++) {
                if (midi.tracks[i].notes.length > 1) {
                    ...
                    myMaterial.diffuseTexture = new BABYLON.Texture("textures/Tiles_Triangles_001_basecolor.jpg", this.gameScene);
                    myMaterial.bumpTexture = new BABYLON.Texture("textures/Tiles_Triangles_001_normal.jpg", this.gameScene);
                    myMaterial.ambientTexture = new BABYLON.Texture("textures/Tiles_Triangles_001_ambientOcclusion.jpg", this.gameScene);
                    myMaterial.diffuseTexture.uScale = 400;
                    ...
                    //var cloudProcText = new BABYLON.StarfieldProceduralTexture("starfield", 1024, this.gameScene);
                    //myMaterial.emissiveTexture = cloudProcText;
                    //myMaterial.backFaceCulling = false;
                    myMaterial.wireframe = true;
                    //myMaterial.emissiveTexture.coordinatesMode = BABYLON.Texture.SKYBOX_MODE;
